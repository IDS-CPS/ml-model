...
for param in params:
    ...
    scaler_src = f"scaler/pit/lstm-{history_size}.gz"
    model_src = f"model/pit/lstm-{history_size}"
    mean_npy = f"npy/pit/lstm/mean-{history_size}.npy"
    std_npy = f"npy/pit/lstm/std-{history_size}.npy"
    ...
    for i in range (len(attack_data)-history_size):
        ...
        if consecutive_counter > time_threshold:
            ...
            if attack_label == 'Attack':
                attack_detected.append(attack_number)
    ...
    attack_set = set(attack_detected)
    print(param, attack_set)

    fpr, tpr, thresholds = roc_curve(real_value, predicted_value)

    grid_results.append({
        "attack_detected": len(attack_set),
    ...
    attack_df.to_csv(f"dataset/pit/lstm-{history_size}-{threshold}-{time_threshold}.csv")
